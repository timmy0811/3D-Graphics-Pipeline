...
void projection::Triangle::applyPerspective()
{
	p1->applyPerspective();
	p2->applyPerspective();
	p3->applyPerspective();
...
void projection::Triangle::moveByValue(sf::Vector3f dir)
{
	p1->moveByValue(dir);
	p2->moveByValue(dir);
	p3->moveByValue(dir);
...
void projection::Triangle::moveToPos(sf::Vector3f pos)
{
	p1->moveToPos(pos);
	p2->moveToPos(pos);
	p3->moveToPos(pos);
...
void projection::Triangle::rotateX(float angle)
{
	p1->rotateX(angle);
	p2->rotateX(angle);
	p3->rotateX(angle);
...
void projection::Triangle::rotateY(float angle)
{
	p1->rotateY(angle);
	p2->rotateY(angle);
	p3->rotateY(angle);
...
void projection::Triangle::rotateZ(float angle)
{
	p1->rotateZ(angle);
	p2->rotateZ(angle);
	p3->rotateZ(angle);
...
void projection::Triangle::rotateX(float angle, sf::Vector3f refPosition)
{
...
void projection::Triangle::rotateY(float angle, sf::Vector3f refPosition)
{
...
void projection::Triangle::rotateZ(float angle, sf::Vector3f refPosition)
{
...
void projection::Triangle::rotateByCamera(float angle, sf::Vector3f refPosition)
{
...
std::vector<projection::AbstractObject*> projection::Triangle::getChildren()
{
...
double projection::Triangle::averageZ()
{
	return (p1->getPosition().z + p2->getPosition().z + p3->getPosition().z) / 3.f;
}

double projection::Triangle::calculateProjectedZ()
{
...
void projection::Triangle::setTexCord(unsigned int point, sf::Vector2f cord)
{
	switch (point) {
	case 1: {
		texCord1_ = cord;
...
sf::Vector2f projection::Triangle::getTexCord(unsigned int point)
{
	switch (point) {
	case 1: {
		return texCord1_;
